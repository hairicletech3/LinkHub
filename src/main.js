...
// ============ Contact card ============
// TODO: put the real contact target here — a phone number opens the dialer:
//   const CONTACT_URL = 'tel:+85512345678';
// Until that's set the card points at Telegram, which is a working link, so
// the card is never dead.
const CONTACT_URL = 'REPLACE_WITH_CONTACT_URL';
const CONTACT_FALLBACK = 'https://t.me/Anclehai';

(function initContact() {
  const card = document.getElementById('contact-card');
  if (!card) return;
  const live = !isPlaceholder(CONTACT_URL);
  card.href = live ? CONTACT_URL : CONTACT_FALLBACK;
  if (!live) {
    const sub = card.querySelector('[data-contact-sub]');
    if (sub) sub.textContent = 'Message us on Telegram';
  }
})();
...
const SOCIALS = [
  ...
  { name: 'Telegram',  icon: 'telegram',  url: 'https://t.me/Anclehai' },
];
